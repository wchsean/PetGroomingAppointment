...
export async function PUT(
  ...
  try {
    ...
    const { customer_name, customer_email, customer_note } = body

    const result = await pool.query(
      `UPDATE grooming.customers 
       SET customer_name = $1, customer_email = $2, customer_note = $3, updated_at = CURRENT_TIMESTAMP
       WHERE id = $4 AND customer_active = true
       RETURNING *`,
      [customer_name, customer_email, customer_note, customerId],
    )
