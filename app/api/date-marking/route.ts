...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<ApiResponse<AvailabilityRule[]>>> {
  try {
    const result = await pool.query(
      'SELECT * FROM grooming.date_marking ORDER BY created_at DESC',
    )
...
export async function DELETE(
  ...
  try {
    ...
    const result = await pool.query(
      'DELETE FROM grooming.date_marking WHERE marking_date = $1 RETURNING date_marking',
      [date],
    )
