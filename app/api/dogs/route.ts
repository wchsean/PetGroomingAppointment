...
export async function POST(
  ...
  try {
    ...
    const customerCheck = await pool.query(
      'SELECT id FROM grooming.customers WHERE id = $1',
      [customer_id],
    )
    ...
    const result = await pool.query(
      `INSERT INTO grooming.dogs 
       (customer_id, dog_name, dog_breed, dog_note, dog_weight, dog_appearance, behavior_profile) 
      ...
      [
        ...
        dog_weight || null,
        dog_appearance || null,
        JSON.stringify(behavior_profile || {}),
      ],
    )
